document.getElementById("addSession").addEventListener('click', validateEmail);
document.getElementById("cancelAddSession").addEventListener('click', clearFormData);
...
document.getElementById("filterData").addEventListener("input", searchSessions, true);
...
function showAddForm(){
    document.getElementById('addSessionForm').removeAttribute('hidden');
...
function showFindForm(){
    ...
    document.getElementById('showAddForm').setAttribute('hidden', true);
    document.getElementById('showFindForm').setAttribute('hidden', true);
    document.getElementById('addSessionForm').setAttribute('hidden', true);
...
function searchSessions() {
    let userTableData = document.querySelector('[id="session-table"]');
    let trList = userTableData.querySelectorAll('tr');
    let find = document.getElementById("findMovieSession");
    let filter = document.getElementById("filterData").value;
    let typeSearch;
    for (let j = 0; j < find.options.length; j++) {
        if (find.options[j].selected) {
            if(find.options[j].value.localeCompare("findByFilmName") == 0){
                typeSearch = "filmName";
            }
    ...
    for(let i = 0; i < trList.length; i++){
        let str = trList[i].querySelector('[id='+typeSearch+']').innerHTML;
...
function clearFormData() {
    let addForm = document.getElementById('addSessionFormTable');
...
function saveUser() {
    ...
    let action = document.getElementById("addSession").name;
